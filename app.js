// Firebase v10.12.2 - Final Secure Initialization
...
// --- Global Variables & Firebase Instances ---
let auth, db;
...
document.addEventListener('DOMContentLoaded', async () => {
    // This async function ensures Firebase is initialized before any other code runs.
    try {
        // Step 1: Fetch the configuration from our secure server endpoint
        ...
        // Step 2: Check if the fetched config is valid
        if (!firebaseConfig.apiKey) {
             throw new Error('Fetched Firebase config is missing API Key.');
        }

        // Step 3: Initialize Firebase with the fetched config
        const app = initializeApp(firebaseConfig);
        auth = getAuth(app);
        db = getFirestore(app);
        
        // Step 4: Now that Firebase is ready, initialize the UI and event listeners
        initializeUIAndEventListeners();

    } catch (error) {
        console.error("Critical Error: Firebase initialization failed.", error);
        showMessage("アプリケーションの起動に失敗しました。ページを再読み込みしてください。", "error");
    }
});

// --- Main UI Logic ---
function initializeUIAndEventListeners() {
    console.log('Firebase is ready. Initializing UI...');
    
    // All the code that was previously in DOMContentLoaded goes here.
    // This ensures it only runs *after* auth and db are available.
    ...
    // Auth State Change Listener is the core of the app's reactive state
    ...
    if (guestLoginBtn) guestLoginBtn.addEventListener('click', () => signInAnonymously(auth).catch(handleAuthError));
    ...
    if (clearChatBtn) clearChatBtn.addEventListener('click', () => {
        if (confirm('チャット履歴をクリアしますか？')) chatMessages.innerHTML = '';
    });
    if (storeConsultationCheckbox) storeConsultationCheckbox.addEventListener('change', (e) => {
    ...
    stepIndicators.forEach(step => step.addEventListener('click', () => {
...
// --- All other functions remain outside, accessible globally ---
...
function appendChatMessage(type, htmlContent) {
    ...
    const contentToShow = isAI ? (typeof marked !== 'undefined' ? marked.parse(htmlContent) : htmlContent) : htmlContent.replace(/\n/g, '<br>');
...
function adjustChatHeight() {
    ...
    if (chatContainer) {
        ...
        const newHeight = window.innerHeight - topOffset - 150;
...
async function saveProfile() {
    ...
    try {
        ...
        document.getElementById('user-display-name').textContent = profileData.displayName || currentUser.email || 'ゲスト';
        showSection(3);
    } catch (e) {
...
function updatePageTexts() {
    ...
    setText('.subtitle', 'headerSubtitle');
    setText('#section-1 h2', 'welcomeTitle');
    setText('#section-1 .section-header p', 'welcomeDesc');
    setText('#login-form h3', 'loginTitle');
    setText('label[for="login-email"]', 'email');
    setText('label[for="login-password"]', 'password');
    setText('#login-btn', 'loginBtn');
    setText('#show-signup-btn', 'showSignupBtn');
    setText('#google-login-btn span', 'googleLoginBtn');
    setText('#guest-login-btn', 'guestLoginBtn');
    setText('#signup-form h3', 'signupTitle');
    setText('label[for="signup-email"]', 'email');
    setText('label[for="signup-password"]', 'password');
    setText('label[for="signup-password-confirm"]', 'passwordConfirm');
    setText('#signup-btn', 'signupBtn');
    setText('#show-login-btn', 'showLoginBtn');
    setText('#section-2 h2', 'profileTitle');
    setText('#section-2 .section-header p', 'profileDesc');
    setText('label[for="display-name"]', 'displayName');
    setText('label[for="nationality"]', 'nationality');
    setText('label[for="primary-language"]', 'primaryLanguage');
    setText('label[for="stay-location"]', 'stayLocation');
    setText('label[for="stay-purpose"]', 'stayPurpose');
    setText('label[for="stay-period"]', 'stayPeriod');
    setText('#save-profile-btn', 'saveProfileBtn');
    setText('#section-3 h2', 'consultationTitle');
    setText('#section-3 .section-header p', 'consultationDesc');
    setText('.category-selector h3', 'categoryTitle');
    setText('#section-4 h2', 'historyTitle');
    setText('#section-4 .section-header p', 'historyDesc');
    setText('#back-to-consultation-btn', 'backToConsultation');
    setText('#export-history-btn', 'exportHistory');
